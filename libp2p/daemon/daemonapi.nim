...
type
  ...
  P2PDaemonFlags* = enum
    DHTServer,     ## Start daemon in DHT server mode
...
proc newDaemonApi*(flags: set[P2PDaemonFlags] = {},
                   ...
                   peersRequired = bootstrapNodes.len,
                   logFile = "",
  ...
  api.process = startProcess(cmd, "", args, env, {poEchoCmd,poStdErrToStdOut, poUsePath, poParentStreams})
...
proc transactMessage(transp: StreamTransport,
  ...
  let res = await transp.write(pb.getString())
  if res != length:
...
proc getPeerInfo(pb: var ProtoBuffer): PeerInfo =
  ...
  while pb.getBytes(2, address) != -1:
    if len(address) != 0:
      ...
      address.setLen(0)
  discard pb.getString(3, result.domain)
  discard pb.getString(4, result.transport)
  discard pb.getVarintValue(5, result.latency)
