...
func makeDHT(ctx context.Context, h host.Host, cfg config) (*IpfsDHT, error) {
	...
	fmt.Println("maxLastSuccessfulOutboundThreshold:",maxLastSuccessfulOutboundThreshold)
...
func (dht *IpfsDHT) FindLocal(id peer.ID) peer.AddrInfo {
	...
	case network.NotConnected:
		if id == dht.host.ID(){
			return peer.AddrInfo{id,dht.host.Network().ListenAddresses(),""}
		}
...
func (dht *IpfsDHT) nearestPeersToQuery(pmes *pb.Message, count int) []peer.ID {
	closer := make([]peer.ID, 0)
	conns := dht.host.Network().Conns()

	for _, c := range conns {
		remotePeer := c.RemotePeer()
		domain, err := dht.peerstore.Get(remotePeer, "Domain")
		...
		closer = append(closer, c.RemotePeer())
	}
	if dht.mode == modeServer {
		nearest := dht.routingTable.NearestPeers(kb.ConvertKey(string(pmes.GetKey())), count)
		closer = append(closer, nearest...)
	}
	return closer
}
