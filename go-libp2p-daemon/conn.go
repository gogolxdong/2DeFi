...
func (d *Daemon) doListPeers(req *pb.Request) *pb.Response {
	...
	for x, conn := range conns {
		...
		transport := conn.RemoteTransport()
		domain, _ := d.host.Peerstore().Get(remotePeer,"Domain")
		//if err != nil {
		//	continue
		//}
		domainName ,_ := domain.(string)
		var latency int64 = int64(d.host.Peerstore().LatencyEWMA(remotePeer))
		peers[x] = &pb.PeerInfo{
			Id:    []byte(remotePeer),
